...
function loadProducts() {
    ...
    fetch('get_products.php')
        ...
        .catch(error => {
            ...
            productList.innerHTML = '<div class="error">An error occurred. Please try again.</div>';
        });
}
...
function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        productItem.innerHTML = `
            <img src="${product.image_path}" alt="${product.title}" style="width: 50px; height: 50px;" onerror="this.onerror=null; this.src='images/placeholder.png';">
...
function addColorInput() {
    ...
    newColorInput.innerHTML = `
        ...
        <button type="button" class="remove-color" onclick="removeColorInput(this)">Remove</button>
    `;
...
function removeColorInput(button) {
    const colorInput = button.parentElement;
    colorInput.remove();
}
    ...
    if (confirm('Are you sure you want to delete this product?')) {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                loadProducts(); // Refresh the product list immediately after deletion
            } else {
